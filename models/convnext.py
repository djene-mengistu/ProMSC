...
class Stem(nn.Sequential):
    def __init__(self, c1, c2, k, s, p=0): #Modified to include padding
        super().__init__(
            nn.Conv2d(c1, c2, k, s, padding=p),
            LayerNorm(c2)
        )
...
class ConvNeXt(nn.Module):     
    def __init__(self, model_name: str = 'T') -> None:
        ...
        self.downsample_layers = nn.ModuleList([
            Stem(3, embed_dims[0], 4, 2, p=1), #Modified to include padding and also the stride from 4 to 2 to make the resolution 112*112 at the first stage
...
#     model = CNNModel('ConvNeXt-S', 4)
#     model.load_state_dict(torch.load('C:/Users/djmen/Desktop/SSL25/models/weights/convnext_small_1k_224_ema.pth', map_location='cpu')['model'], strict=False)
