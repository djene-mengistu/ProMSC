...
class DEFDataset(Dataset):
    def __init__(self, img_path, mask_path, X, mean, std, transform=None, is_unlabeled=False):
        ...
        self.mean = mean
        self.std = std
        # self.num_classes = num_classes  # Number of foreground classes (excluding background)
    ...
    def __getitem__(self, idx):
        ...
        # # Compute class label as binary vector [0, 1, 0, 1, ...] for classes 1 to num_classes
        unique_classes = np.unique(mask)
        class_label = np.zeros(self.num_classes, dtype=np.int64)  # Shape: [num_classes]
        for cls in unique_classes:
            if cls > 0 and cls <= 3:  # Exclude background (0) [3:NEU-Seg, 5:MTD, 6:DAGM]
                class_label[cls - 1] = 1  # Class index 1 maps to label[0], etc.
        ...
        if self.is_unlabeled:
            ...
            # Generate boundary map after spatial transforms
            # boundary = get_boundary_map(mask)
            # boundary_map = get_labeled_boundary_map_mc(mask)

            # Apply weak non-spatial transforms
            weak_aug = self.weak_transform(image=img_spatial)
            # img_weak = Image.fromarray(weak_aug['image'])
            img_weak = weak_aug['image']

            # Apply strong non-spatial transforms
            strong_aug = self.strong_transform(image=img_spatial)
            # img_strong = Image.fromarray(strong_aug['image'])
            ...
            # boundary = torch.from_numpy(boundary_map).long()#float() / 255.0  # Normalize to [0, 1] for model
            class_label = torch.from_numpy(class_label).long()
            
            return img_weak, img_strong, mask, class_label
        else:
            ...
            # Generate boundary map after transforms
            # boundary = get_boundary_map(mask)
            # boundary_map = get_labeled_boundary_map_mc(mask)
            img = T.Compose([
                T.ToTensor(),
            ...
            mask = torch.from_numpy(mask).long()
            # boundary = torch.from_numpy(boundary_map).long()# / 255.0  # Normalize to [0, 1] for model
            class_label = torch.from_numpy(class_label).long()
...
def load_DEF_dataloaders(image_path_train, mask_path_train,
    ...
    X_train, X_untrain = train_test_split(df['id'].values, test_size=unlabeled_ratio, random_state=69)
    
    # if len(X_train) < len(X_untrain):
    #     X_train = np.random.choice(X_train, size=len(X_untrain), replace=True)
