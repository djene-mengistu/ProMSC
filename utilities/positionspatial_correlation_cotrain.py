...
class FeatureConsistencyLoss(nn.Module):
    ...
    def forward(self, weak_features, strong_features):
        ...
        for weak, strong in zip(weak_features, strong_features):
            # Adjust strong features to match weak features channel dimension
            if strong.shape[1] != weak.shape[1]:
                strong = F.adaptive_avg_pool2d(strong, (weak.shape[2], weak.shape[3]))  # Match spatial size first
                strong = F.conv2d(strong, torch.ones(weak.shape[1], strong.shape[1], 1, 1, device=strong.device) / strong.shape[1])
            
            # weak_detached = weak.detach() #CHECK
